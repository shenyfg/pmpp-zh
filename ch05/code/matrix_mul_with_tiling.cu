...
int main(int argc, char **argv) {
  ...
  printf("\n正在执行矩阵乘法（带tiling）...\n");
  CHECK_CUDA_ERROR(cudaEventRecord(start));
  matrixMul_with_tiling_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B,
                                                                   d_P, size);
  CHECK_CUDA_ERROR(cudaEventRecord(stop));
  CHECK_CUDA_ERROR(cudaGetLastError());
  CHECK_CUDA_ERROR(cudaEventSynchronize(stop));

  float milliseconds_with_tiling = 0;
  CHECK_CUDA_ERROR(
      cudaEventElapsedTime(&milliseconds_with_tiling, start, stop));
  printf("带tiling的执行时间: %.3f ms\n", milliseconds_with_tiling);

  // 计算加速比
  float speedup = milliseconds_no_tiling / milliseconds_with_tiling;
  printf("\n性能提升: %.2fx\n", speedup);
  printf("性能提升百分比: %.2f%%\n", (speedup - 1.0f) * 100.0f);
